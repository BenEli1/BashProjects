...
player2_pick() {
  ...
  echo ""
  if [[ $player_2_guess =~ ^-?[0-9]+$ ]] && [ $player_2_guess -le $points2 ] && (("$player_2_guess" >= 0"")); then
    ...
    if [ $player_2_guess -gt $player_1_guess ]; then
      case $pos in
      0)
        pos=$((pos - 1))
        line=" |       |   O   #       |       | "
...
player1_pick() {
  ...
  if [[ $player_1_guess =~ ^-?[0-9]+$ ]] && [ $player_1_guess -le $points1 ] && (("$player_1_guess" >= 0"")); then
    player2_pick
  else
    echo "NOT A VALID MOVE !"
    player1_pick
...
checkWinner() {
  ...
  if [ "$points1" -eq "0" ] && (("$points2" > "0")) || [ "$pos" -eq "-3" ] && $game_on; then
    echo "PLAYER 2 WINS !"
    game_on=false
  fi
  #if players 2 has no points and 1 has so player 1 won or if the ball reached 3 spot
  if [ "$points2" -eq "0" ] && (("$points1" > "0")) || [ "$pos" -eq "3" ] && $game_on; then
    echo "PLAYER 1 WINS !"
    game_on=false
  fi

  #if both are at 0 points so its a draw
  if [ "$points1" -eq "0" ] && [ "$points2" -eq "0" ] && $game_on; then
    case $pos in
